...
#     #for currentMouse in ['pss3570_Champ_2022-09-04_1','pss3570_Chip_2022-09-04_1','pss3570_Caesar_2022-09-04_1']:
...
#                 #     for AP_IDX,CCF_AP in enumerate(CCF_AP_Slices):
#                 #         for ML_IDX,CCF_ML in enumerate(CCF_ML_Slices):
#                 #             if np.sum(pooledClusterCounts[structIDX,AP_IDX,ML_IDX,:]) == 0:
#                 #                 pooledClusterCounts[structIDX,AP_IDX,ML_IDX,:] = np.full((maxCluster), np.nan)


#                 # layerH3means = [[np.nanmean(pooledClusterCounts[areaIDX,:,:,H3IDX]) for H3IDX in range(newMaxCluster)] for areaIDX in range(structNum)]
#                 # layerH3_SEM = [[np.nanstd(pooledClusterCounts[areaIDX,:,:,H3IDX])/np.sqrt(np.count_nonzero(~np.isnan(pooledClusterCounts[areaIDX,:,:,H3IDX]))) for H3IDX in range(newMaxCluster)] for areaIDX in range(structNum)]
...
#             #######
#             ### Tau
#             #tempFilePath = r'C:\Users\lai7370\Projects\TauProcessing\SwiftSync\PreFeb26'
#             #tau_area_processed = np.load(os.path.join(tempFilePath,currentMouse+'_tau_area_processed.npy'))
#             #areaLebelsSet = np.load(os.path.join(tempFilePath,currentMouse+'_areaLabelsSet.npy'))
...
#             # fig,axes=plt.subplots(1,1,figsize=(4,4))
#             # ax=axes
#             # ax.scatter(np.arange(0,maxCluster,1),clusterWeights[0][:],color='black')
#             # ax.set_xlabel('H3 Index')
#             # ax.set_ylabel('H3 Weight')
#             # plt.title('H3 Weights for Tau Area Reconstruction ('+titleName+', Z-Scored)\n'+currentMouse)
#             # plt.savefig(os.path.join(projectPath,currentMouse+'_'+titleName+'_H3WeightsForTau.pdf'),dpi=600,bbox_inches="tight")
