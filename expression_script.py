...
from packages.expression_model_hyperparams import *
...
from utils.param_shortcut import get_wf_preproc_param_set_ids

# old name: expressionRegression.py
...
def parameters_for_line(line):
    if line[-2:] == '8m':
        param_set_ids = get_wf_preproc_param_set_ids('latest')
    ...
    if line[-2:] == '96':
        param_set_ids = get_wf_preproc_param_set_ids('default')
    ...
    param_dict['mc_param_set_id'] = param_set_ids['mc_param_set_id']
    param_dict['pixel_param_set_id'] = param_set_ids['pixel_param_set_id']
    param_dict['dff_param_set_id'] = param_set_ids['dff_param_set_id']

    return param_dict



def run_transcript_tau_GLMs(args_dict):

    line_selection = args_dict['line_selection']
    gene_limit = args_dict['gene_limit']
    restrict_merfish_imputed_values = args_dict['restrict_merfish_imputed_values']
    tau_pool_size_array_full = args_dict['tau_pool_size_array_full']
    n_splits = args_dict['n_splits']
    alpha_params = args_dict['alpha_params']
    plotting = args_dict['plotting']
    num_precision = args_dict['num_precision']
    alpha_precision = args_dict['alpha_precision']
    verbose = args_dict['verbose']
    predictor_order = args_dict['predictor_order']
    regressions_to_start = args_dict['regressions_to_start']
    max_iter = args_dict['max_iter']
    variable_management = args_dict['variable_management']
    plotting_conditions = args_dict['plotting_conditions']
    arg_parse_test = args_dict['arg_parse_test']
    job_task_id = args_dict['job_task_id']
    bootstrapping_scale = args_dict['bootstrapping_scale']
    min_pool_size = args_dict['min_pool_size']
    preprocessing_only = args_dict['preprocessing_only']
    output_to_repo = args_dict['output_to_repo']

    functional_set = ['renan_set', 'extended_set', 'extra_set_only'][0] # sessions to choose from for defining functional tau, options are 'renan_set', 'extended_set', 'extra_set_only'
    log_tau = False
    ...
    if True: # just a placeholder, should probably just remove the above line and the conditional runs below...
        ...
        save_path_OSs, data_path = pathSetter(output_to_repo=output_to_repo)

        if not output_to_repo:
            save_path = save_path_OSs[my_os == 'Windows']
        else:
            save_path_OSs = [save_path_OSs, save_path_OSs]
            save_path = save_path_OSs
        ...
        for resolution in [10,25,100]:
            output_dir = os.path.join(data_path,f'nrrd{resolution}')
            ...
            annotation, meta = ref_space_cache.get_annotation_volume() #in browser navigate to the .nrrd file and download manually, if not working automatically (which it should be)
        ...
        if functional_set == 'extra_set_only':
            key_list_intothevoid = get_keys(extra_sessions_list)

        gene_data_dense, pilot_gene_names, fn_clustid, fn_CCF = pilotLoader(save_path, data_path)
        merfish_CCF_Genes, all_merfish_gene_names, gene_categories, used_merfish_genes_longnames = merfishLoader(save_path, data_path, pilot_gene_names, restrict_merfish_imputed_values, gene_limit)

        param_dict = parameters_for_line(line_selection)
        all_tau_CCF_coords, CCF25_bregma, CCF25_lambda = load_tau_CCF(line=line_selection, keys=key_list_intothevoid, task='IntoTheVoid', param_dict=param_dict, tau_cutoff=59.9)
        ...
        with open(os.path.join(save_path, f'merfishImputed_usedLongGeneNames.pickle'), "wb") as file:
            pickle.dump(used_merfish_genes_longnames, file, protocol=pickle.HIGHEST_PROTOCOL)
        ...
        if plotting:
            ...
            plotting_subsample = 1
            img = ax.scatter(all_tau_CCF_coords[0,::plotting_subsample], all_tau_CCF_coords[1,::plotting_subsample], s=1, alpha=0.5, c=all_tau_CCF_coords[2,::plotting_subsample], cmap='cool')
            ...
            fig_1_b_merfish_dict = {}
            fig_1_b_merfish_dict['mlCCF_per_cell_H2layerFiltered'] = mlCCF_per_cell_H2layerFiltered
            fig_1_b_merfish_dict['apCCF_per_cell_H2layerFiltered'] = apCCF_per_cell_H2layerFiltered
            # dump fig_1_b_merfish_dict into a pickle file for reference
            with open(os.path.join(save_path, f'fig_1_b_merfish_dict.pickle'), "wb") as file:
                pickle.dump(fig_1_b_merfish_dict, file, protocol=pickle.HIGHEST_PROTOCOL)
        ...
        for layerNames,numLayers,resolution,datasetName in zip([layer_names_list[order] for order in predictor_order],[num_layers_list[order] for order in predictor_order],[resolution_list[order] for order in predictor_order],[dataset_name_list[order] for order in predictor_order]):
            ...
            for tauPoolSize in tau_pool_size_array:
                ...
                for layerIDX in range(numLayers):
                    ...
                    # dump pooledTauCCF_coords into pickle file for reference
                    with open(os.path.join(save_path, 'fig_1_b_tau_data.pickle'), "wb") as file:
                        pickle.dump(pooledTauCCF_coords, file, protocol=pickle.HIGHEST_PROTOCOL)
                    fig, ax = plt.subplots(1,1,figsize=(2.5,3))
                    cmap = plt.get_cmap('cool')
                    sc = ax.scatter(pooledTauCCF_coords[layerIDX][:,0], pooledTauCCF_coords[layerIDX][:,1], c=pooledTauCCF_coords[layerIDX][:,2], s=2, cmap=cmap)
...
if __name__ == '__main__':
    ...
    args_dict = script_args(from_command_line=True)
