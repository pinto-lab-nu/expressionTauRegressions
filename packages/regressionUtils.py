...
def layerRegressions(response_dim,n_splits,highMeanPredictorIDXs,x_data,y_data,layerNames,regressionConditions,cell_region,alphaParams,max_iter,verbose=False):
    ...
    betas_over_alpha = [np.zeros((alphas.shape[0],response_dim,highMeanPredictorIDXs[layerIDX].shape[0])) for layerIDX in range(numLayers)]
        ...
        for foldIDX,(train_index, test_index) in enumerate(kfold.split(y_data[layerIDX])):
            ...
            if verbose:
                ...
                for alphaIDX,alpha in enumerate(alphas):
                    ...
                    betas_over_alpha[layerIDX][alphaIDX,:,:] = lasso.coef_
                ...
                bestR2[layerIDX, foldIDX] = lasso_cv.score(test_x, test_y)
                
                betas_over_alpha[layerIDX] = [] # this is a verbose output, so we skip it if not needed
                loss_history_test_global, loss_history_train_global, dual_gap_history_global = [], [], [] # also verbose outputs, can skip these


            lasso = Lasso(alpha=bestAlpha[layerIDX, foldIDX], fit_intercept=False, max_iter=max_iter)
            lasso.fit(train_x, train_y)
    ...
    return best_betas, betas_over_alpha, bestAlpha, alphas, tauPredictions, bestR2, loss_history_test_global, loss_history_train_global, dual_gap_history_global
