...
def plot_regressions(generate_plots, generate_summary, gene_categories, lineSelection, structList, areaColors, plottingConditions, params, paths, titles, model_vals, plotting_data, regressions_to_plot='All', paper_plotting=False, cross_layers='All', super_verbose=False):
    ...
    if predictorTitle == 'Gene Expression':
        ...
        for layerIDX in range(numLayers):
            ...
            for gene_idx, gene_text in enumerate(predictorNamesArray[highMeanPredictorIDXs[layerIDX]]):
                # if gene_text in gene_categories['metabolic_genes']:
                #     gene_color = 'black'
                #     other_genes.append(gene_text)
                # elif gene_text in gene_categories['ion_channel_genes']:
                #     gene_color = 'green'
                # elif gene_text in gene_categories['glu_receptor_genes']:
                #     gene_color = 'blue'
                # elif gene_text in gene_categories['gaba_receptor_genes']:
                #     gene_color = 'purple'
                # elif gene_text in gene_categories['plast_conn_genes']:
                #     gene_color = 'orange'
                # else:
                #     gene_color = 'black'
                #     other_genes.append(gene_text)
        ...
        legend_handles = [
            mpatches.Patch(color="green", label='Ion channels and ion homeostasis'),
            mpatches.Patch(color="#4C42FF", label='Glutamatergic transmission'),
            mpatches.Patch(color="purple", label='GABAergic transmission'),
            mpatches.Patch(color="#FF9900", label='Synaptic function, connectivity, and plasticity, neuronal development'),
            mpatches.Patch(color='#000000', label='Others')
        ]

        category_colors = [
            "green",        # Ion channels and ion homeostasis
            "purple",       # GABAergic transmission
            "#4C42FF",      # Glutamatergic transmission
            "#FF9900",      # Synaptic function, connectivity, and plasticity, neuronal development
            "#000000"       # Others
        ]
    ...
    for reconstruction_type in np.asarray(['spatial', 'tau', 'X_tauRes', 'XCCF_tau'])[regressions_to_plot]:
        ...
        if generate_plots:
            ...
            plt.ylabel(f'{recon_type} Reconstruction $R^2$ ($\pm$ {n_splits}-fold SD)')
            if not paper_plotting:
            ...
            if spatialReconstruction and (predictorPathSuffix == 'GenePredictors') and (meanExpressionThresh == 0):
                ...
                for fileType in ['.pdf','.svg']:
                    ...
                    fig, axes = plt.subplots(len(cross_layers), len(cross_layers))
                    ...
                    for fig_idx_0, layerIDX0 in enumerate(cross_layers):
                        for fig_idx_1, layerIDX1 in enumerate(cross_layers):
                            ...
                            if layerIDX0 == numLayers-1:
                                axes[fig_idx_0, fig_idx_1].set_xlabel(f"{layerNames[layerIDX1]} Tau $\\beta$", fontname=paper_font)
                    plt.savefig(os.path.join(savePath,f'crossLayer_{datasetName}_{typeTitle}_Beta_Correlations_{file_append}{fileType}'), format=fileType[1:], dpi=600, bbox_inches='tight')
                    plt.close()
            ...
            for layerIDX,layerName in enumerate(layerNames):
                ...
                gene_color_list_layer_sorted = list(np.array(gene_color_list[layerIDX])[[sorted_coef[layerIDX][dim,:]]].flatten())
                ...
                fig, axes = plt.subplots(response_dim,1,figsize=(5,3))
                if not paper_plotting:
